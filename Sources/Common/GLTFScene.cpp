                           ...
                           ImageCallback imageCallback)
{
    ...
    {
        ProcessAnimation(model, anim, _sceneChannels.size(),
                         _sceneSamplers.size());
    }
...
void GLTFScene::ProcessMesh(const tinygltf::Model& model,
                            ...
                            VertexFormat format, const std::string& name)
{
    ...
    if (mesh.indices > -1)
    {
        ...
        switch (indexAccessor.componentType)
        {
            case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT:
                ...
                std::memcpy(&_u32Buffer[0],
                            &buffer.data[indexAccessor.byteOffset +
                                         bufferView.byteOffset],
                            indexAccessor.count * sizeof(unsigned int));
                _indices.insert(_indices.end(),
                                std::make_move_iterator(_u32Buffer.begin()),
                                std::make_move_iterator(_u32Buffer.end()));
                break;
            case TINYGLTF_PARAMETER_TYPE_UNSIGNED_SHORT:
                _u16Buffer.resize(indexAccessor.count);
                std::memcpy(&_u16Buffer[0],
                            &buffer.data[indexAccessor.byteOffset +
                                         bufferView.byteOffset],
                            indexAccessor.count * sizeof(unsigned short));
                _indices.insert(_indices.end(),
                                std::make_move_iterator(_u16Buffer.begin()),
                                std::make_move_iterator(_u16Buffer.end()));
                break;
            case TINYGLTF_PARAMETER_TYPE_UNSIGNED_BYTE:
                _u8Buffer.resize(indexAccessor.count);
                std::memcpy(&_u8Buffer[0],
                            &buffer.data[indexAccessor.byteOffset +
                                         bufferView.byteOffset],
                            indexAccessor.count * sizeof(unsigned char));
                _indices.insert(_indices.end(),
                                std::make_move_iterator(_u8Buffer.begin()),
                                std::make_move_iterator(_u8Buffer.end()));
                break;
            default:
                std::cerr << "Unknown index component type : "
                          << indexAccessor.componentType << " is not supported"
                          << std::endl;
                return;
        }
    }
    ...
    //! POSITION
    {
        [[maybe_unused]] bool result =
            GetAttributes<glm::vec3>(model, mesh, _positions, "POSITION");
    ...
    if (static_cast<int>(format & VertexFormat::Normal3))
    {
        if (!GetAttributes<glm::vec3>(model, mesh, _normals, "NORMAL"))
        {
            ...
            std::vector<glm::vec3> meshNormals(resultMesh.vertexCount,
                                               glm::vec3(0.0f));
            ...
            _normals.insert(_normals.end(),
                            std::make_move_iterator(meshNormals.begin()),
                            std::make_move_iterator(meshNormals.end()));
        }
    }
    ...
    if (static_cast<int>(format & VertexFormat::Tangent4))
    {
        ...
        {
            //! Implementation in "Foundations of Game Engine Development :
            //! Volume2 Rendering"
            std::vector<glm::vec3> tangents(resultMesh.vertexCount,
                                            glm::vec3(0.0f));
            std::vector<glm::vec3> bitangents(resultMesh.vertexCount,
                                              glm::vec3(0.0f));
            for (size_t i = 0; i < resultMesh.indexCount; i += 3)
            {
                //! Local index
            ...
            for (unsigned int i = 0; i < resultMesh.vertexCount; ++i)
            {
                ...
                glm::vec3 tangent =
                    glm::normalize(t - n * glm::vec3(glm::dot(n, t)));
                //! Calculate the handedness
                float handedness =
                    (glm::dot(glm::cross(t, b), n) > 0.0f) ? 1.0f : -1.0f;
                _tangents.emplace_back(tangent.x, tangent.y, tangent.z,
                                       handedness);
            }
        }
    }
    ...
    if (static_cast<int>(format & VertexFormat::Color4))
    {
        ...
        {
            _colors.insert(_colors.end(), resultMesh.vertexCount,
                           glm::vec4(1.0f));
        }
    }
...
void GLTFScene::ProcessNode(const tinygltf::Model& model, int nodeIdx,
                            int parentIndex)
{
    ...
    glm::mat4 worldMat =
        (parentIndex != -1 ? _sceneNodes[parentIndex].world : glm::mat4(1.0f)) *
        GetLocalMatrix(newNode);
    ...
    else if (node.extensions.find(KHR_LIGHTS_PUNCTUAL_EXTENSION_NAME) !=
             node.extensions.end())
    {
        GLTFLight light;
        const auto& ext =
            node.extensions.find(KHR_LIGHTS_PUNCTUAL_EXTENSION_NAME)->second;
...
bool GLTFScene::UpdateAnimation(size_t animIndex, double timeElapsed)
{
         ...
         ch < anim.channelCount + anim.channelIndex; ++ch)
    {
        ...
        for (size_t i = 0; i < sampler.inputs.size() - 1; ++i)
        {
            ...
            {
                const float keyframe = std::max(
                    0.0f, (elapsed - sampler.inputs[i]) /
                ...
                if (keyframe <= 1.0f)
                {
                    switch (channel.path)
                    {
                        case GLTFChannel::Path::Translation:
                            node.translation = Interpolation::Lerp(
                                sampler.outputs[i], sampler.outputs[i + 1],
                                keyframe);
                            break;
                        case GLTFChannel::Path::Rotation:
                            node.rotation = glm::normalize(glm::slerp(
                                ...
                                glm::quat(sampler.outputs[i + 1].w,
                        ...
                        case GLTFChannel::Path::Scale:
                            node.scale = Interpolation::Lerp(
                                sampler.outputs[i], sampler.outputs[i + 1],
                                keyframe);
                            break;
                        case GLTFChannel::Path::Weights:
                            std::cerr << "[GLTFScene::UpdateAnimation] Weights "
                                         "not implemented yet"
                                      << std::endl;
                            return false;
                            break;
...
void GLTFScene::ProcessAnimation(const tinygltf::Model& model,
                                 const tinygltf::Animation& anim,
                                 std::size_t channelOffset,
                                 std::size_t samplerOffset)
{
...
void GLTFScene::ProcessChannel(const tinygltf::AnimationChannel& channel)
{
    ...
    else
    {
        ...
        std::cerr
            << "[GLTFScene::ProcessAnimation] Unknown channel target path : "
            << channel.target_path << '\n';
        return;
    }
                               ...
                               const tinygltf::AnimationSampler& sampler)
{
    ...
    else
    {
        ...
        std::cerr << "[GLTFScene::ProcessAnimation] Unknown sampler "
    ...
    //! Process sampler outputs
    {
        ...
        else
        {
            ...
            std::cerr << "[GLTFScene::ProcessAnimation] Unknown sampler output "
                         "type : "
                      << accessor.type << '\n';
            return;
        }
...
void GLTFScene::CalculateSceneDimension()
{
    ...
    for (const auto& node : _sceneNodes)
    {
        for (size_t meshIdx : node.primMeshes)
        {
            ...
            bbMin = { std::min(bbMin.x, localMin.x),
                      std::min(bbMin.z, localMin.z),
                      std::min(bbMin.z, localMin.z) };
            bbMax = { std::max(bbMax.x, localMax.x),
                      std::max(bbMax.z, localMax.z),
                      std::max(bbMax.z, localMax.z) };
        }
    }
...
void GLTFScene::ImportMaterials(const tinygltf::Model& model)
{
    ...
    for (const auto& mat : model.materials)
    {
        ...
        material.alphaMode =
            mat.alphaMode == "MASK" ? 1 : (mat.alphaMode == "BLEND" ? 2 : 0);
            ...
            mat.extensions.end())
        {
            ...
            const auto& ext =
                mat.extensions
                    .find(KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS_EXTENSION_NAME)
                    ->second;
            GetValue<glm::vec4>(ext, "diffuseFactor",
                                material.specularGlossiness.diffuseFactor);
            GetValue<float>(ext, "glossinessFactor",
                            material.specularGlossiness.glossinessFactor);
            GetValue<glm::vec3>(ext, "specularFactor",
                                material.specularGlossiness.specularFactor);
            GetTextureID(ext, "diffuseTexture",
                         material.specularGlossiness.diffuseTexture);
            GetTextureID(ext, "specularGlossinessTexture",
                         material.specularGlossiness.specularGlossinessTexture);
        }

        // KHR_texture_transform
        if (pbr.baseColorTexture.extensions.find(
                KHR_TEXTURE_TRANSFORM_EXTENSION_NAME) !=
            pbr.baseColorTexture.extensions.end())
        {
            const auto& ext = pbr.baseColorTexture.extensions
            ...
            glm::mat3 scale =
                glm::mat3(tt.scale.x, 0, 0, 0, tt.scale.y, 0, 0, 0, 1);
            tt.uvTransform = scale * rotation * translation;
        }

        // KHR_materials_unlit
        if (mat.extensions.find(KHR_MATERIALS_UNLIT_EXTENSION_NAME) !=
            mat.extensions.end())
        {
        ...
        if (mat.extensions.find(KHR_MATERIALS_CLEARCOAT_EXTENSION_NAME) !=
            mat.extensions.end())
        {
            const auto& ext =
                mat.extensions.find(KHR_MATERIALS_CLEARCOAT_EXTENSION_NAME)
                    ->second;
            GetValue<float>(ext, "clearcoatFactor", material.clearcoat.factor);
            GetTextureID(ext, "clearcoatTexture", material.clearcoat.texture);
            GetValue<float>(ext, "clearcoatRoughnessFactor",
                            material.clearcoat.roughnessFactor);
            GetTextureID(ext, "clearcoatRoughnessTexture",
                         material.clearcoat.roughnessTexture);
            GetTextureID(ext, "clearcoatNormalTexture",
                         material.clearcoat.normalTexture);
        }

        // KHR_materials_sheen
        if (mat.extensions.find(KHR_MATERIALS_SHEEN_EXTENSION_NAME) !=
            mat.extensions.end())
        {
            const auto& ext =
                mat.extensions.find(KHR_MATERIALS_SHEEN_EXTENSION_NAME)->second;
            GetValue<glm::vec3>(ext, "sheenColorFactor",
                                material.sheen.colorFactor);
            GetTextureID(ext, "sheenColorTexture", material.sheen.colorTexture);
            GetValue<float>(ext, "sheenRoughnessFactor",
                            material.sheen.roughnessFactor);
            GetTextureID(ext, "sheenRoughnessTexture",
                         material.sheen.roughnessTexture);
        }

        // KHR_materials_transmission
        if (mat.extensions.find(KHR_MATERIALS_TRANSMISSION_EXTENSION_NAME) !=
            mat.extensions.end())
        {
            const auto& ext =
                mat.extensions.find(KHR_MATERIALS_TRANSMISSION_EXTENSION_NAME)
                    ->second;
            GetValue<float>(ext, "transmissionFactor",
                            material.transmission.factor);
            GetTextureID(ext, "transmissionTexture",
