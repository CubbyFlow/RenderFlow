...
void BoundingBox::Merge(const BoundingBox& bb)
{
    ...
    {
        this->_lowerCorner.x =
            std::min(bb._lowerCorner.x, this->_lowerCorner.x);
        this->_lowerCorner.y =
            std::min(bb._lowerCorner.y, this->_lowerCorner.y);
        this->_lowerCorner.z =
            std::min(bb._lowerCorner.z, this->_lowerCorner.z);

        this->_upperCorner.x =
            std::max(bb._upperCorner.x, this->_upperCorner.x);
        this->_upperCorner.y =
            std::max(bb._upperCorner.y, this->_upperCorner.y);
        this->_upperCorner.z =
            std::max(bb._upperCorner.z, this->_upperCorner.z);
    }
}
